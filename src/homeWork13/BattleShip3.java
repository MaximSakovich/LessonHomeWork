...
// Метод функционирования игры
        private void play() {
            ...
            while (totalShips > 0) {
                ...
                if (result == HIT_SYMBOL) {
                    System.out.println("Попадание, корабль подбит!");
                    playerBoard[row][col] = HIT_SYMBOL;
                    totalShips--;
                } else if (result == MISS_SYMBOL) {
                    System.out.println("Промах. Нужно стараться!!!");
                    playerBoard[row][col] = MISS_SYMBOL;
                }
            }
    ...
    private char takeShot(char[][] board, int row, int col) {
        if (!isValidShot(board, row, col)) {
